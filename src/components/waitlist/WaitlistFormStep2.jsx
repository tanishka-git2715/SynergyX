...
import { saveWaitlistStep2 } from '@/lib/firebase';
...
const WaitlistFormStep2 = ({ formData, handleInputChange, waitlistId, onStepComplete }) => {
  ...
  const handleRoleSelect = async (roleId) => {
    ...
    try {
      const { success, error } = await saveWaitlistStep2(waitlistId, { ...formData, role: roleId });
      
      if (success) {
        toast({
          title: "Role selected!",
          description: "Let's continue with your interests.",
        });
        onStepComplete?.();
      } else {
        toast({
        ...
        handleInputChange('role', formData.role);
      }
    } catch (error) {
      toast({
        variant: "destructive",
